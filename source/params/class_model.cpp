...
void class_model::randomize_lattice() {
    for(int i = 0; i < L; i++) {
        for(int j = 0; j < L; j++) { lattice(i, j) = 2 * rn::uniform_integer_1() - 1; }
    }
}
...
double class_model::get_E() {
    double E = 0;
    for(int i = 0; i < L; i++) {
        for(int j = 0; j < L; j++) { E += -J * lattice(i, j) * sum_neighbours(i, j); }
    }

    return E / 2.0;
}

double class_model::get_M() { return (double) lattice.sum(); }

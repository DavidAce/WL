...
class_worker::class_worker(int &id, int &size)
    : world_ID(id), world_size(size), model(constants::L), finish_line(false), t_total(profiling_total, 3, "Total Time"), t_print(profiling_print, 3, "Time"),
      t_sweep(profiling_sweep, 3, "t_sweep"), t_swap(profiling_swap, 3, "t_swap"), t_merge(profiling_merge, 3, "t_merge"),
      t_setup_team(profiling_setup_team, 3, "t_setup"), t_sync_team(profiling_sync_team, 3, "t_sync"), t_divide_range(profiling_divide_range, 3, "t_divr"),
      t_check_convergence(profiling_check_convergence, 3, "t_conv"), t_acceptance_criterion(profiling_acceptance_criterion, 3, "t_accr") {
...
void class_worker::set_rate_increment() {
    ...
    for(int i = 0; i < dos.rows(); i++) {
        if((dos.row(i) != 0).any()) { dos_height += 1; }
    }

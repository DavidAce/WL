...
namespace parallel {
    void swap2(class_worker &worker) {
        ...
        MPI_Sendrecv(&worker.E_min_local, 1, MPI_DOUBLE, swap_partner, 102, &E_min_partner, 1, MPI_DOUBLE, swap_partner, 102, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
        MPI_Sendrecv(&worker.E_max_local, 1, MPI_DOUBLE, swap_partner, 103, &E_max_partner, 1, MPI_DOUBLE, swap_partner, 103, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
        MPI_Sendrecv(&worker.dos(worker.E_idx, worker.M_idx), 1, MPI_DOUBLE, swap_partner, 104, &dos_partner, 1, MPI_DOUBLE, swap_partner, 104, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
        ...
        if(myTurn) {
            if(swap) {
                P_swap = exp(worker.dos(worker.E_idx, worker.M_idx) - worker.dos(E_idx_partner, M_idx_partner) + dos_partner - dos_myown);
            } else {
                P_swap = 0;
            }
    ...
    void divide_global_range_uniform(class_worker &worker) {
        ...
        // Add a little extra if there are too many workers (Add nothing if world_size == 2, and up to local_volume if world_size == inf)
        ...
        int team_id;
        if(!worker.team->is_active()) { worker.team->set_defaults(); }
        team_id = worker.team->get_team_id();
        // The teams on the interior can have half the overlap range
        if(team_id == 0) {
            overlap_range = overlap_range / 1;
        } else if(team_id == constants::num_teams - 1) {
            overlap_range = overlap_range / 1;
        } else {
            overlap_range = overlap_range / 2;
        }
    ...
    void divide_global_range_dos_area(class_worker &worker) {
        ...
        if(worker.team->get_team_id() == 0) {
            overlap_range = overlap_range / 2;
        } else if(worker.team->get_team_id() == constants::num_teams - 1) {
            overlap_range = overlap_range / 2;
        } else {
            overlap_range = overlap_range / 4;
        }
        E_min_local_idx = math::area_idx(worker.dos_total, worker.E_bins_total, worker.M_bins_total, worker.team->get_team_id() * local_range - overlap_range);
        E_max_local_idx =
            math::area_idx(worker.dos_total, worker.E_bins_total, worker.M_bins_total, (worker.team->get_team_id() + 1) * local_range + overlap_range);
    ...
    void divide_global_range_dos_volume(class_worker &worker) {
        ...
        E_min_local_idx =
            math::volume_idx(worker.dos_total, worker.E_bins_total, worker.M_bins_total, worker.team->get_team_id() * local_range - overlap_range);
        E_max_local_idx =
            math::volume_idx(worker.dos_total, worker.E_bins_total, worker.M_bins_total, (worker.team->get_team_id() + 1) * local_range + overlap_range);

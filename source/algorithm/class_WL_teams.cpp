...
class_WL_teams::class_WL_teams(class_worker &worker_ref, int num_teams_)
: worker(worker_ref)
{
...
//    debug_print_team_commander("\n \n");
    ...
    for (auto i = 0; i < random_walk_recv.size(); i++) {
...
void class_WL_teams::setup_teams(){
    ...
    for (int w = 0; w < worker.world_size; w++) {
        if (w == worker.world_ID){
            ...
            if(worker.finish_line == 1){
                team_forbidden(team_id_new) = 1;
            }
        }

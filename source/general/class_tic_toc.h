...
class class_profiling {
    ...
    inline void tic() {
        if(profiling) { delta_tic = high_resolution_clock::now(); }
    }
    ...
    inline void print_delta() {
        if(profiling) { cout << setprecision(print_precision) << name << duration_cast<duration<double>>(delta_time).count(); }
    }
    template<typename rep = double, typename time_type = std::ratio<1, 1>>
    inline void print_delta() {
        if(profiling) { cout << name << duration_cast<duration<rep, time_type>>(delta_time).count(); }
    }

    inline void print_total() {
        if(profiling) { cout << setprecision(print_precision) << name << duration_cast<duration<double>>(total_time).count(); }
    }

    template<typename rep = double, typename time_type = std::ratio<1, 1>>
    inline void print_total() {
        if(profiling) { cout << name << duration_cast<duration<rep, time_type>>(total_time).count(); }
    }

    inline void print_total_reset() {
        if(profiling) {
            cout << setprecision(print_precision) << name << duration_cast<duration<double>>(total_time).count();
            reset();
        }
    }
    ...
    inline void print_total_reset() {
        if(profiling) {
            cout << name << duration_cast<duration<rep, time_type>>(total_time).count();
            reset();
        }
    }
